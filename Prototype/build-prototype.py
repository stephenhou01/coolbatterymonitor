...
"p.unlock_title":  {"zh-Hans":"越用越值钱","en":"It gets better the longer you keep it","ja":"使うほど価値が増します","de":"Je länger genutzt, desto wertvoller"},
"p.unlock_head":   {"zh-Hans":"这是一台随身的信号收集器 —— 有些答案只有时间能给","en":"A signal recorder you carry with you — some answers only time can give",
                    "ja":"持ち歩く信号レコーダー —— 時間だけが出せる答えがあります","de":"Ein Signalrekorder in Ihrer Tasche — manche Antworten kann nur Zeit geben"},
"p.unlock_sub":    {"zh-Hans":"下面的门槛不是人为设的关卡，是数据真的还不够。快照工具永远给不出右侧那几项 —— 它们必须靠时间攒。",
                    "en":"These thresholds are not artificial gates — the data genuinely is not there yet. A snapshot tool can never produce the items on the right; they have to be accumulated.",
                    "ja":"以下のしきい値は人為的な制限ではなく、データが実際にまだ足りないためです。スナップショット型ツールでは右側の項目は決して得られません。時間をかけて蓄積するしかありません。",
                    "de":"Diese Schwellen sind keine künstlichen Hürden — die Daten fehlen schlicht noch. Ein Snapshot-Tool kann die rechten Punkte nie liefern; sie müssen erst gesammelt werden."},
"p.ul_day0":       {"zh-Hans":"装上就有","en":"Day 0","ja":"導入直後","de":"Ab Tag 0"},
"p.ul_dayn":       {"zh-Hans":"第 {n} 天","en":"Day {n}","ja":"{n} 日目","de":"Tag {n}"},
"p.ul_0":          {"zh-Hans":"健康与容量真相","en":"Health & capacity truth","ja":"健康度と容量の実像","de":"Zustand & Kapazität"},
"p.ul_0d":         {"zh-Hans":"容量去向、温度分档、历史极值 —— IOKit 一读就有","en":"Where capacity went, temperature bands, all-time extremes — one read of IOKit","ja":"容量の行方・温度帯・過去の極値 —— IOKit を一度読むだけ","de":"Kapazitätsverteilung, Temperaturbänder, Extremwerte — ein IOKit-Lesevorgang"},
"p.ul_3":          {"zh-Hans":"首批场景画像","en":"First scenario profiles","ja":"最初のシーン画像","de":"Erste Szenarienprofile"},
"p.ul_3d":         {"zh-Hans":"「你看视频时约 14W」这类结论开始出现","en":"Statements like “video ≈ 14W for you” start appearing","ja":"「動画視聴時は約14W」といった結論が出始めます","de":"Aussagen wie „Video ≈ 14 W bei Ihnen“ erscheinen"},
"p.ul_7":          {"zh-Hans":"充电习惯评分","en":"Charging-habit score","ja":"充電習慣スコア","de":"Ladegewohnheits-Score"},
"p.ul_7d":         {"zh-Hans":"满充存放、充电温度、深度区间都攒够一周","en":"A week of standby, charge temperature and depth data","ja":"満充電放置・充電温度・充電深度が一週間分","de":"Eine Woche Standby-, Ladetemperatur- und Tiefendaten"},
"p.ul_14":         {"zh-Hans":"剩余寿命换算成年份","en":"Remaining life in years","ja":"残り寿命を年数で","de":"Restlebensdauer in Jahren"},
"p.ul_14d":        {"zh-Hans":"实测出你自己的循环速率，才敢把「剩余 N 次循环」说成年份","en":"Only after measuring your own cycle rate can “N cycles left” become years","ja":"あなた自身のサイクル速度を実測して初めて「残りNサイクル」を年数に換算できます","de":"Erst mit Ihrer gemessenen Zyklusrate lässt sich „N Zyklen“ in Jahre umrechnen"},
"p.ul_30":         {"zh-Hans":"场景库与变化归因","en":"Scenario library & attribution","ja":"シーンライブラリと変化の帰因","de":"Szenarienbibliothek & Zuordnung"},
"p.ul_30d":        {"zh-Hans":"「续航为什么变短了」能给出具体原因","en":"“Why did runtime drop” gets a concrete answer","ja":"「なぜ持ちが悪くなったか」に具体的な答えが出せます","de":"„Warum sinkt die Laufzeit“ bekommt eine konkrete Antwort"},
"p.ul_90":         {"zh-Hans":"衰减曲线与内阻趋势","en":"Fade curve & resistance trend","ja":"劣化曲線と抵抗の推移","de":"Verlustkurve & Widerstandstrend"},
"p.ul_90d":        {"zh-Hans":"你这块电池自己的老化速率 —— 没有任何快照工具能给","en":"This pack's own ageing rate — no snapshot tool can ever provide it","ja":"このパック固有の劣化速度 —— スナップショット型では決して得られません","de":"Die Alterungsrate genau dieses Packs — für Snapshot-Tools unerreichbar"},
"p.unlock_note":   {"zh-Hans":"<b>怎么收集的：</b>只用两样数据 —— 进程列表（proc_listpids）和系统功耗（BatteryData.SystemPower），<b>都不需要任何授权弹窗</b>。三条纪律：① 按电量计 56 秒的刷新节奏采样，采得更快只是在复制样本、虚假抬高置信度；② 场景需达到最小样本量才展示；③ 用进程组合而非 CPU 最高的那个来识别场景 —— 实测 Spotlight 索引 CPU 94%，功耗却只有 9.3W，<b>CPU 最高的进程不等于耗电的进程</b>。",
                    "en":"<b>How it is collected:</b> just two inputs — the process list (proc_listpids) and system power (BatteryData.SystemPower). <b>Neither requires any permission prompt.</b> Three rules: (1) sample at the gauge's ~56 s cadence, since faster sampling merely duplicates readings and inflates confidence; (2) a scenario is shown only once it has enough samples; (3) identify scenarios by the process mix, not the top-CPU process — Spotlight indexing was measured at 94% CPU while power stayed at 9.3 W, so <b>the busiest process is not the power-hungry one</b>.",
                    "ja":"<b>収集方法：</b>使うのは 2 つだけ —— プロセス一覧（proc_listpids）とシステム消費電力（BatteryData.SystemPower）。<b>いずれも許可ダイアログは不要です。</b>三つの原則：① 電量計の約56秒の更新間隔に合わせて採取する（それより速く採ってもサンプルを複製し信頼度を偽って高めるだけ）、② 十分なサンプル数に達したシーンのみ表示する、③ CPU 最上位ではなくプロセスの組み合わせでシーンを判定する —— Spotlight のインデックス作成は CPU 94% でも消費電力は 9.3W。<b>CPU が最も高いプロセスが最も電気を食うとは限りません。</b>",
                    "de":"<b>Wie erhoben wird:</b> nur zwei Quellen — die Prozessliste (proc_listpids) und die Systemleistung (BatteryData.SystemPower). <b>Beide ohne jeden Berechtigungsdialog.</b> Drei Regeln: (1) im ~56-s-Takt des Gauges messen, denn schneller zu messen dupliziert nur Werte und täuscht Sicherheit vor; (2) ein Szenario wird erst ab ausreichender Stichprobe gezeigt; (3) Szenarien über die Prozesskombination erkennen, nicht über den CPU-Spitzenreiter — Spotlight-Indexierung lag bei 94 % CPU, die Leistung aber bei 9,3 W. <b>Der CPU-lastigste Prozess ist nicht der stromhungrigste.</b>"},
...
DATA = dict(ra=RA, cells=CELLS, qmax=QMAX, design=design, fcc=fcc, cur=cur, reserve=reserve,
            ...
            observedDays=1, peakW=round(abs(LT['MaximumDischargeCurrent'])*volt/1000), wh=round(fcc*volt/1000,1), tempAvg=round(LT['AverageTemperature']/10,1),
